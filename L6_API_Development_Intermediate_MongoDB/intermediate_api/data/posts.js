...
const exportedMethods = {
  ...
  async getPostsByTag(tag) {
    ...
    const postCollection = await posts();
    return await postCollection.find({ tags: tag }).toArray(); // finds all the blog posts with a specific tag
  },
  async addPost(title, body, posterId, tags) {
    ...
    // addresses tags as an optional value
  ...
  async updatePostPut(id, updatedPost) {
    ...
    // for a PUT, if no tags are supplied, they are overwritten to an empty array (PATCH doesn't do this)
  ...
  async updatePostPatch(id, updatedPost) {
    ...
    // checks each possible supplied field, as PATCH can take in any number of fields (not all like PUT)
    ...
    let newPost = await postCollection.findOneAndUpdate(
      { _id: new ObjectId(id) },
      { $set: updatedPostData }, // reassigns the old field values to the new ones ONLY FOR THOSE SUPPLIED
      { returnDocument: "after" }
    );
  ...
  async renameTag(oldTag, newTag) {
    ...
    let firstUpdate = { // add the new tag to the documents found with the old tag above
      $addToSet: { tags: newTag },
    };

    let secondUpdate = { // then we will remove the old tag from every found document
      $pull: { tags: oldTag },
    };
    const postCollection = await posts();
    ...
    // find all the blog posts with the old tag, pushes the new tag, pulls the old tag
